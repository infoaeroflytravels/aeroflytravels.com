...
interface PriceBreakdownProps {
  ...
  govtFee: number; // e.g. 6750
  aeroFee: number; // e.g. 1000
  aeroFeeDiscounted?: number; // optional (e.g. 0)
  qty?: number; // e.g. 1
}
...
const PriceBreakdown: React.FC<PriceBreakdownProps> = ({
  ...
  const total = govtFee * qty + (aeroFeeDiscounted > 0 ? aeroFeeDiscounted : aeroFee);
      ...
      <div className="pt-16 relative flex h-full w-full flex-col place-items-center justify-center gap-8 rounded-2xl border bg-white border-t">
        <div className="relative z-[2] w-full select-none px-2">
          <div className="transition w-full rounded-xl">
            <div className="flex flex-row items-center gap-2 px-2">
              <p className="mb-0 font-semibold">
                {destination} Visa Price Breakdown
              </p>
            </div>
                    ...
                    {aeroFeeDiscounted > 0 ? (
                      <>
                        <p className="text-gray-500 line-through">
                          {currencySymbol}
                          {aeroFee.toLocaleString()}
                        </p>
                        <p className="font-semibold text-primary">
                          {currencySymbol}
                          {aeroFeeDiscounted.toLocaleString()} <span>for now</span>
                        </p>
                      </>
                    ) : (
          ...
          <div className="mt-2 w-full px-4 pb-6">
            <div className="rounded-xl border bg-white p-0 transition-all duration-300">
